"""
TEST 2d: Long-Only Reversal Strategy

Trading Strategy:
- Calculate SPY/TLT ratio return from day 1 to day before last 5 (signal period)
- If SPY outperforms TLT (positive ratio return): LONG TLT during last 5 days
- If TLT outperforms SPY (negative ratio return): LONG SPY during last 5 days
- FLAT all other times

This is a long-only version of test2c, betting on reversal at month-end.
...
print("="*80)
print("TEST 2d: LONG-ONLY REVERSAL STRATEGY")
print("="*80)
print()
...
# For each month, determine which asset to long during last 5 days
...
for period, group in df.groupby('year_month'):
    ...
    if len(first_days) > 0:
        ...
        # Set position for last 5 days of this month
        # If SPY outperformed (positive ratio), go LONG TLT (the underperformer)
        # If TLT outperformed (negative ratio), go LONG SPY (the underperformer)
...
results.to_csv('../data/test2d_performance.csv', index=False)
print(f"\nResults saved to: ../data/test2d_performance.csv")

# Save daily data
df[['spy_return', 'tlt_return', 'ratio_return', 'trading_day', 'is_last_5',
    'position', 'strategy_return', 'cum_return', 'drawdown']].to_csv('../data/test2d_daily_data.csv')
print(f"Daily data saved to: ../data/test2d_daily_data.csv")
...
ax1.set_title(f'Long-Only Reversal Strategy (Long Underperformer at Month-End)\n'
...
plt.tight_layout()
plt.savefig('../plots/test2d_long_only_reversal.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test2d_long_only_reversal.png")

# Print sample to verify logic
print("\n" + "="*80)
print("SAMPLE POSITIONS (First 60 trading days)")
print("="*80)
print(f"{'Date':<12} {'Day':>4} {'Last5?':>7} {'Position':>10} {'SPY Ret':>9} {'TLT Ret':>9} {'Strat Ret':>10}")
print("-"*80)
for i in range(min(60, len(df))):
    row = df.iloc[i]
    print(f"{row.name.date()} {row['trading_day']:>4} "
          f"{'YES' if row['is_last_5'] else 'NO':>7} {row['position']:>10} "
