"""
TEST 3: Volatility-Filtered Dynamic Portfolio

Only include pairs where volatility is high enough to overcome borrow costs.
...
- Include pair only if: (volatility / k) - total_borrow > 0
...
PAIRS = [
    ...
    ('SSO', 'SDS', 2),    # 2x
    ('TMF', 'TMV', 3),    # 3x
    ('NUGT', 'DUST', 2),  # 2x (changed from 3x in 2020)
    ('YINN', 'YANG', 3),  # 3x
    ('BOIL', 'KOLD', 2),  # 2x
    ('UCO', 'SCO', 2),    # 2x
]
...
print("="*80)
print("TEST 3: VOLATILITY-FILTERED DYNAMIC PORTFOLIO")
print("="*80)
print(f"Max pairs: {len(PAIRS)}")
print()

# Calculate k values for each leverage
# k represents how many vol points needed per 1% borrow cost
k_values = {}
for leverage in [2, 3]:
...
for bull, bear, leverage in PAIRS:
    ...
    k = k_values[leverage]
    # Convert vol to percentage (vol is in decimal form, e.g., 0.579 = 57.9%)
    # Filter: vol% > k * avg_borrow%
    pair_df['vol_pct'] = pair_df[f'vol_{bull}'] * 100
    pair_df['required_vol'] = k * pair_df['avg_borrow']
...
summary.to_csv('../data/test3_volatility_filtered_results.csv', index=False)
print(f"\nResults saved to: ../data/test3_volatility_filtered_results.csv")
...
ax1.set_title(f'Volatility-Filtered Portfolio\nCAGR: {cagr:.2%} | Sharpe: {sharpe:.2f} | Max DD: {max_dd:.2%}',
              fontsize=12, pad=15)
...
plt.tight_layout()
plt.savefig('../plots/test3_volatility_filtered_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test3_volatility_filtered_portfolio.png")
