"""
TEST 3b: Volatility-Filtered with 5% Edge Requirement
...
EDGE_REQUIREMENT = 5.0  # 5% additional edge requirement

client = FMPClient()

print("="*80)
print("TEST 3b: VOLATILITY-FILTERED WITH 5% EDGE REQUIREMENT")
print("="*80)
...
for bull, bear, leverage in PAIRS:
    ...
    # Calculate threshold with 5% edge requirement
    k = k_values[leverage]
...
summary.to_csv('../data/test3b_edge_filter_results.csv', index=False)
print(f"\nResults saved to: ../data/test3b_edge_filter_results.csv")
...
ax1.set_title(f'Volatility-Filtered Portfolio (5% Edge Required)\nCAGR: {cagr:.2%} | Sharpe: {sharpe:.2f} | Max DD: {max_dd:.2%}',
              fontsize=12, pad=15)
...
plt.tight_layout()
plt.savefig('../plots/test3b_edge_filter_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test3b_edge_filter_portfolio.png")
