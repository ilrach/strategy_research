"""
TEST 3e: Volatility-Filtered with 20% Edge Requirement (2020+)

Same as TEST 3d but only backtest from 2020 onwards.
Only include pairs where volatility is high enough to overcome borrow costs PLUS 20% annual edge.
...
EDGE_REQUIREMENT = 20.0  # 20% additional edge requirement

client = FMPClient()

print("="*80)
print("TEST 3e: VOLATILITY-FILTERED WITH 20% EDGE (2020+)")
print("="*80)
print(f"Max pairs: {len(PAIRS)}")
print(f"Edge requirement: {EDGE_REQUIREMENT}% additional hurdle")
print(f"Period: 2020-01-01 onwards")
print()
...
# Load BIL
print("Loading BIL...")
bil_data = client.get_historical_prices('BIL', '2020-01-01', '2025-10-29')
...
for bull, bear, leverage in PAIRS:
    ...
    # Load prices
    bull_prices = pd.DataFrame(client.get_historical_prices(bull, '2020-01-01', '2025-10-29'))
    ...
    bear_prices = pd.DataFrame(client.get_historical_prices(bear, '2020-01-01', '2025-10-29'))
    ...
    # Calculate threshold with 20% edge requirement
    k = k_values[leverage]
...
summary.to_csv('../data/test3e_edge_filter_20pct_2020_results.csv', index=False)
print(f"\nResults saved to: ../data/test3e_edge_filter_20pct_2020_results.csv")
...
ax1.set_title(f'Volatility-Filtered Portfolio (20% Edge Required)\nCAGR: {cagr:.2%} | Sharpe: {sharpe:.2f} | Max DD: {max_dd:.2%}',
              fontsize=12, pad=15)
...
plt.tight_layout()
plt.savefig('../plots/test3e_edge_filter_20pct_2020_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test3e_edge_filter_20pct_2020_portfolio.png")
