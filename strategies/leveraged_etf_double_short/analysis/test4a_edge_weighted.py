"""
TEST 4: Edge-Proportional Weighting

Instead of equal weighting all pairs above threshold, weight each pair
proportionally to its expected edge.

Edge = (vol% / k) - avg_borrow%

If edge <= 0, weight = 0 (don't include)
If edge > 0, weight proportional to edge

All weights normalized to sum to -100% total short exposure.
"""
...
print("="*80)
print("TEST 4: EDGE-PROPORTIONAL WEIGHTING")
print("="*80)
print(f"Max pairs: {len(PAIRS)}")
print(f"Weighting: Proportional to expected edge")
print()
...
for date in bil_df.index:
    # Find which pairs have data and positive edge on this date
    ...
    if len(available_pairs_with_edge) == 0:
        # No pairs with positive edge - hold cash only
...
summary.to_csv('../data/test4_edge_weighted_results.csv', index=False)
print(f"\nResults saved to: ../data/test4_edge_weighted_results.csv")
...
plt.tight_layout()
plt.savefig('../plots/test4_edge_weighted_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test4_edge_weighted_portfolio.png")
