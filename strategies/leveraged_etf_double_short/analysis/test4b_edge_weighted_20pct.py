"""
TEST 4b: Edge-Proportional Weighting with 20% Edge Threshold

Similar to TEST 4a, but only include pairs with edge > 20%.
Then weight each qualifying pair proportionally to its expected edge.

Edge = (vol% / k) - avg_borrow%

If edge <= 20%, weight = 0 (exclude from portfolio)
If edge > 20%, weight proportional to edge
...
print("="*80)
print("TEST 4b: EDGE-PROPORTIONAL WEIGHTING (20% THRESHOLD)")
print("="*80)
...
for date in bil_df.index:
    ...
    if len(available_pairs_with_edge) == 0:
        # No pairs with edge > threshold - hold cash only
...
summary.to_csv('../data/test4b_edge_weighted_20pct_results.csv', index=False)
print(f"\nResults saved to: ../data/test4b_edge_weighted_20pct_results.csv")
...
plt.tight_layout()
plt.savefig('../plots/test4b_edge_weighted_20pct_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test4b_edge_weighted_20pct_portfolio.png")
