"""
TEST 5: Ablation Study - No Cash Yield

Same as TEST 4b but assuming zero yield on cash.
This tests if the strategy has edge above the risk-free rate.

Differences from TEST 4b:
- Cash earns 0% (not BIL - 100bps)
- Still uses 20% edge threshold
- Still uses edge-proportional weighting
"""
...
print("="*80)
print("TEST 5: ABLATION - NO CASH YIELD")
print("="*80)
...
summary.to_csv('../data/test5_ablation_no_cash_yield_results.csv', index=False)
print(f"\nResults saved to: ../data/test5_ablation_no_cash_yield_results.csv")
...
plt.tight_layout()
plt.savefig('../plots/test5_ablation_no_cash_yield_portfolio.png', dpi=300, bbox_inches='tight')
print(f"Plot saved to: ../plots/test5_ablation_no_cash_yield_portfolio.png")
